...
from robin.supply.constants import (
    DEFAULT_LIFT_CONSTRAINTS, EDGE_TYPE_SAME_MARKET, EDGE_TYPE_SAME_AGENT, EDGE_TYPE_DEST_ORIGIN, MAX_DEPTH
)
...
class Service:
    ...
    Attributes:
        ...
        origin_station (Station): Origin station of the service.
        destination_station (Station): Destination station of the service.
    ...
    def __init__(
        ...
        self.origin_station = self.line.stations[0]
        self.destination_station = self.line.stations[-1]
...
class Journey:
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f'n_transfers={self.n_transfers}, '
...
class Supply:
    ...
    def _get_same_agent_edges(self, service_to_idx: Dict[str, int]) -> Tuple[List[Tuple[int, int]], List[int]]:
        """
        Private method to get edges between services from the same agent (internal coordination).
        
        Args:
            service_to_idx (Dict[str, int]): Mapping from service IDs to numerical indices.
            
        Returns:
            Tuple[List[Tuple[int, int]], List[int]]: List of edge tuples and edge types for same agent relationships.
        """
        edge_index = []
        # Group services by their TSP and date
        tsp_groups = {}
        for service in self.services:
            key = (service.tsp.id, service.date)
            if key not in tsp_groups:
                tsp_groups[key] = []
            tsp_groups[key].append(service)
        
        # Create edges between services from the same TSP
        for tsp_services in tsp_groups.values():
            if len(tsp_services) > 1:
                for i in range(len(tsp_services)):
                    for j in range(i + 1, len(tsp_services)):
                        service1_idx = service_to_idx[tsp_services[i].id]
                        service2_idx = service_to_idx[tsp_services[j].id]
                        edge_index.append((service1_idx, service2_idx))
                        edge_index.append((service2_idx, service1_idx))
        edge_type = [EDGE_TYPE_SAME_AGENT] * len(edge_index)
        return edge_index, edge_type

    def _get_same_destination_origin_edges(self, service_to_idx: Dict[str, int]) -> Tuple[List[Tuple[int, int]], List[int]]:
        """
        Private method to get edges between services with destination-origin connection (direct coordination).
        
        Connects services where one service's destination matches another service's origin station.
        For example, service A→B connects to service B→C.
        
        Args:
            service_to_idx (Dict[str, int]): Mapping from service IDs to numerical indices.
            
        Returns:
            Tuple[List[Tuple[int, int]], List[int]]: List of edge tuples and edge types for destination-origin relationships.
        """
        edge_index = []
        # Group services by their origin station and date
        origin_groups = {}
        for service in self.services:
            key = (service.origin_station.id, service.date)
            if key not in origin_groups:
                origin_groups[key] = []
            origin_groups[key].append(service)

        # For each service A→B, find services B→X on the same day and create directed edges A→B→X
        for service1 in self.services:
            destination_id = service1.destination_station.id
            key = (destination_id, service1.date)
            if key in origin_groups:
                # Found services that start where service1 ends on the same day
                for service2 in origin_groups[key]:
                    # Don't connect service to itself
                    if service1.id != service2.id:
                        service1_idx = service_to_idx[service1.id]
                        service2_idx = service_to_idx[service2.id]
                        edge_index.append((service1_idx, service2_idx))
        edge_type = [EDGE_TYPE_DEST_ORIGIN] * len(edge_index)
        return edge_index, edge_type

    def _get_same_market_edges(self, service_to_idx: Dict[str, int]) -> Tuple[List[Tuple[int, int]], List[int]]:
        """
        Private method to get edges between services in the same market (direct competition).
        
        Args:
            service_to_idx (Dict[str, int]): Mapping from service IDs to numerical indices.
            
        Returns:
            Tuple[List[Tuple[int, int]], List[int]]: List of edge tuples and edge types for same market relationships.
        """
        edge_index = []
        # Group services by their market (origin, destination) and date
        market_groups = {}
        for service in self.services:
            market = (service.origin_station.id, service.destination_station.id, service.date)
            if market not in market_groups:
                market_groups[market] = []
            market_groups[market].append(service)
        
        # Create edges between services in the same market
        for market_services in market_groups.values():
            if len(market_services) > 1:
                for i in range(len(market_services)):
                    for j in range(i + 1, len(market_services)):
                        service1_idx = service_to_idx[market_services[i].id]
                        service2_idx = service_to_idx[market_services[j].id]
                        edge_index.append((service1_idx, service2_idx))
                        edge_index.append((service2_idx, service1_idx))
        edge_type = [EDGE_TYPE_SAME_MARKET] * len(edge_index)
        return edge_index, edge_type
    ...
    @lru_cache(maxsize=None)
    def get_services_edges(self, exclude_edge_type: Union[int, None] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Get the edge index and type of the services graph.
        
        The edge index is a tensor of shape (2, num_edges) where each column represents an edge
        between services and the two rows represent the source and destination service nodes.

        The edge type is a tensor of shape (num_edges,) where each element represents
        the type of the corresponding edge in the edge index.
        
        Two services are connected if they have:
        0. Same market (direct competition): Same origin and destination stations
        1. Same agent (internal coordination): Same Train Service Provider (TSP)
        2. Same destination-origin (direct coordination): One service's destination matches another's origin

        Args:
            exclude_edge_type (Union[int, None]): If specified, edges of this type will be excluded from the graph.

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Edge index and edge type of the services graph with
                shape (2, num_edges) and (num_edges,) respectively.
        """
        # Create a mapping from services to numerical indices
        service_to_idx = {service.id: idx for idx, service in enumerate(self.services)}
        
        # Get edges from all relationship types
        same_market_edges, same_market_types = self._get_same_market_edges(service_to_idx)
        same_agent_edges, same_agent_types = self._get_same_agent_edges(service_to_idx)
        same_destination_origin_edges, same_destination_origin_types = self._get_same_destination_origin_edges(service_to_idx)

        # Exclude edges of the specified type
        if exclude_edge_type == EDGE_TYPE_SAME_MARKET:
            same_market_edges, same_market_types = [], []
        elif exclude_edge_type == EDGE_TYPE_SAME_AGENT:
            same_agent_edges, same_agent_types = [], []
        elif exclude_edge_type == EDGE_TYPE_DEST_ORIGIN:
            same_destination_origin_edges, same_destination_origin_types = [], []

        # Combine all edges
        edge_index = same_market_edges + same_agent_edges + same_destination_origin_edges
        edge_type = same_market_types + same_agent_types + same_destination_origin_types

        # Reorder edge types from 0 to num_edge_types - 1 after excluding edge types
        if exclude_edge_type is not None and edge_type:
            unique_edge_types = sorted(set(edge_type))
            edge_type_mapping = {et: idx for idx, et in enumerate(unique_edge_types)}
            edge_type = [edge_type_mapping[et] for et in edge_type]

        # Convert the edge index and type to tensors with shape (2, num_edges)
        if not edge_index:
    ...
    def filter_journeys(self, origin: str, destination: str, date: datetime.date, max_depth: int = MAX_DEPTH) -> List[Journey]:
        ...
        while possible_paths:
            ...
            # Get services already used in the current path
            path_services = {service for _, service in path}
    ...
    def filter_services_by_tsp_active(self, tsp_id: str, date: datetime.date) -> List[Service]:
        """
