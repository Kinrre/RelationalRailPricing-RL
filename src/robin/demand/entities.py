...
class Passenger:
    ...
    def get_seats(
        ...
        for i, service in enumerate(journey.services):
            ...
            # Journeys are calculated with the line information, but it is possible that 
            # the service does not have ticket prices for a specific origin-destination pair
            if (origin, destination) not in service.prices:
            ...
            for seat in service.prices[(origin, destination)].keys():
                ...
                tickets_available = service.tickets_available(origin, destination, seat, self.purchase_date)
    ...
    def get_utility(
    ...
    ) -> Tuple[np.ndarray[Seat], Dict[str, np.ndarray[float]]]:
        """
        ...
        Returns:
            ...
            seats (Dict[str, np.ndarray[float]]): The seats of the journey with the utility and price.
        """
